...
import static com.markswell.inventoryservice.application.core.domain.enums.SaleEvent.*;
...
public class DebitInventoryUseCase implements DebitInventoryInputPort {
    ...
    public void debit(Sale sale) {
        try {
            ...
            sendToKafkaInventoryOutputPort.send(sale,  INVENTORY_PREPARED);
        } catch (InsufficientProductQuantityException e) {
            sendToKafkaInventoryOutputPort.send(sale, INVENTORY_ERROR);
            throw e;
        }
